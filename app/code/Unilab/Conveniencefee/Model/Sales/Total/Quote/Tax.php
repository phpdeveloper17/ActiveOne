 ...
 */
namespace Unilab\Conveniencefee\Model\Sales\Total\Quote;
...
class Tax extends \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
    ...
    public function __construct(
        \Unilab\Conveniencefee\Extrafee\Helper\Data $helperData,
    ...
    protected function _roundTotals(\Magento\Quote\Model\Quote\Address\Total $address)
    {
        ...
        foreach ($totalCodes as $totalCode) {
			...
			if($address->getQuote()->getPayment()->getMethod() == "dragonpay"){
				$ConFee   = $this->helperData->getConveniencefeeValue() / $this->gettaxRateFinal($address->getCustomerId());
				$TaxAdded = $this->helperData->getConveniencefeeValue() - $ConFee;
			}else{
				$TaxAdded = 0;
				
			}			

            $roundedAmount = $this->_calculator->round($exactAmount + $totalDelta +  $TaxAdded);
     ...
     * @return  Mage_Tax_Model_Sales_Total_Quote
     */
    protected function _calculateShippingTax(\Magento\Quote\Model\Quote\Address\Total $address, $taxRateRequest)
    {
     ...
     * @return  Mage_Tax_Model_Sales_Total_Quote
     */
    protected function _unitBaseCalculation(\Magento\Quote\Model\Quote\Address\Total $address, $taxRateRequest)
    {
        $items = $this->_getAddressItems($address);
        $itemTaxGroups  = array();
    ...
    protected function _calcUnitTaxAmount(\Magento\Sales\Model\Quote\Item\AbstractItem $item, $rate)
{
     ...
     * @return  Mage_Tax_Model_Sales_Total_Quote
     */
    protected function _rowBaseCalculation(\Magento\Quote\Model\Quote\Address\Total $address, $taxRateRequest)
    {
        $items = $this->_getAddressItems($address);
        $itemTaxGroups  = array();
     ...
     * @return  Mage_Tax_Model_Sales_Total_Quote
     */
    protected function _totalBaseCalculation(\Magento\Quote\Model\Quote\Address\Total $address, $taxRateRequest)
    {
        $items          = $this->_getAddressItems($address);
     ...
     * @param   float $rate
     */
    protected function _saveAppliedTaxes(\Magento\Quote\Model\Quote\Address\Total $address,
