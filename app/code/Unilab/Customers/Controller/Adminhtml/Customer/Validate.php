...
class Validate extends \Magento\Customer\Controller\Adminhtml\Index
{
    ...
    protected function _validateCustomer($response)
    {
        ...
        try {
            ...
            $checkOrigData = $customerObj
                        ->addAttributeToFilter('employee_id',$submittedData['employee_id'])
                        ->load()->count();
            
            $validate=false;
            if(isset($submittedData['entity_id'])){ // check if id is set, this condition is for edit only
                $checkDataExistEdit = $customerObj
                ->addAttributeToFilter('employee_id',$submittedData['employee_id'])
                ->addFieldToFilter('entity_id', $submittedData['entity_id'])
                ->load();
                $checkDataExistEdit = count($checkDataExistEdit->getData());
            ...
            if($validate){
                $response->setMessages(['0'=>'Duplicate Employee ID '.$submittedData['employee_id']]);
                $response->setError(1);
            }
