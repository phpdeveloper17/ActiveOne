...
class ListProduct extends \Magento\Catalog\Block\Product\AbstractProduct implements IdentityInterface
{
    ...
    protected function _getProductCollection()
    {
        if(!$this->getRequest()->getParam('q')){
            $visibility = [];
            $visibility = [2,4];
            $this->catalogSession->setPricelistProductVisibility($visibility);
        }
    ...
    private function initializeProductCollection()
    {
        ...
        $visibilityPP = $this->catalogSession->getPricelistProductVisibility();
        $visibilityPP = implode(',',$visibilityPP);
        $collection = $layer->getProductCollection();
        
        $collection->addAttributeToSelect('*');
        $collection->getSelect()->columns(['price_index.website_id','price_index.customer_group_id'])
            ->joinInner(
                ...
                ['pp' => 'rra_pricelistproduct'],
                 'c.name = pp.pricelist_id AND pp.product_sku = e.sku',
                ['pp.pricelist_id', 'pp.product_sku','pp.visibility as product_visibility']
            )->where('pp.visibility IN('.$visibilityPP.')');
        // echo "<pre>";
