 ...
 */
namespace Unilab\Checkout\Block\Cart\Item;
    ...
    \Magento\Framework\DataObject\IdentityInterface
{
   ...
   /**
    ...
    public function getPrescriptionId($item)
    {
        $connection   = $this->getConnection();

        $sqlQuoteItem = $connection->select()->from('quote_item', array('prescription_id')) 
                ->where('item_id=?', $item->getId());

        $prescription = $connection->fetchRow($sqlQuoteItem);

        return $prescription['prescription_id'];
    }

    public function getUnilabDc($item)
    {
        $product = $this->productRepository->getById($item->getProductId());

        return $product->getData('unilab_dc');
    }
	
	public function getUnilabMoq($item)
	{
		$product = $this->productRepository->getById($item->getProductId());
		$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/productmoq.log');
		$logger = new \Zend\Log\Logger();
		$logger->addWriter($writer);
		$logger->info(json_encode($product->getData()));
        return $product->getUnilabMoq();
	}
