   ...
   public function addData($data)
    ...
    public function processResponse(){
		...
		if (strlen($SECURE_SECRET) > 0 && $this->getData("vpc_TxnResponseCode") != "7" && $this->getData("vpc_TxnResponseCode") != "No Value Returned") 
		{
		    ...
		    if (strtoupper($vpc_Txn_Secure_Hash) == $securhash) {
		        ...
		        // later.
		        $hashValidated = $this->_coreHelper->__('Success');
		    } else {
		        // Secure Hash validation failed, add a data field to be displayed
		        // later.
		        $hashValidated = $this->_coreHelper->__('Invalid Hash');
		        $errorExists = true;
		    }
		} 
		...
		if(!$errorExists && $this->getData("vpc_TxnResponseCode") == "0" && $this->_getOrder()->getState() == Order::STATE_PROCESSING)
		{
			...
			$this->getCoreSession()->addSuccess($this->_coreHelper->__('Your transaction ID is '.$this->getData('vpc_ReceiptNo')))
		...
		else
		{
			...
			$this->updatePayment(true,\Magento\Sales\Model\Order::STATE_NEW);
			...
			throw new \LogicException($hashValidated);
		}
    }
    ...
    {	
        ...
        try {
            if(!$this->_getOrder()->canInvoice())
            {
            	// throw new \LogicException($this->_coreHelper->__('Cannot create an invoice without products.'));
            }
            $invoice = $this->_objectManager->get('\Magento\Sales\Model\Service\InvoiceService', $this->_getOrder())->prepareInvoice();
            if (!$invoice->getTotalQty()) {
            	// throw new \LogicException($this->_coreHelper->__('Cannot create an invoice without products.'));
